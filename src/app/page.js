...
import { ShieldAlert, Activity, Database, Wind, FileText, Pipette, Trash2, Calculator, AlertTriangle, RefreshCcw, Save, History, Search, Info, Sliders, X } from 'lucide-react';
...
export default function ForensicGradeV9() {
  ...
  const auditResult = useMemo(() => {
    ...
    const entities = Object.values(registry).map(e => {
      // RECONCILED LOGIC FIX: Potential is 0 if Tobacco is 0
  ...
  // SUM TOTALS FOR SEARCHED ENTITIES
      ...
      <div className="max-w-[1600px] mx-auto mb-8 flex flex-col lg:flex-row items-center gap-6 bg-white border border-slate-300 p-6 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4 mr-auto">
          ...
          <div>
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-slate-500 italic">Audit Logic & Guide Fully Restored</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 bg-slate-100 px-6 py-3 rounded-2xl border-2 border-slate-200">
           <div className="flex items-center gap-2 font-black uppercase text-[10px] text-black">
             <Sliders size={18} className="text-blue-700"/> Risk Threshold
           </div>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          ...
          <button onClick={() => {setRawData([]); setUrl(''); setSearchTerm('');}} className="p-2.5 text-slate-400 hover:text-red-700 bg-slate-100 border border-slate-200 rounded-xl transition-colors"><RefreshCcw size={20}/></button>
        </div>
      </div>
        ...
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500">
          <div className="flex justify-between items-center border-b-2 border-slate-200">
            <div className="flex gap-10 text-sm font-black uppercase tracking-widest">
              <button onClick={() => setActiveTab('country')} className={`pb-4 transition-colors ${activeTab === 'country' ? 'text-blue-700 border-b-4 border-blue-700' : 'text-slate-400'}`}>Country Intel</button>
              ...
              <button onClick={() => setActiveTab('reports')} className={`pb-4 transition-colors ${activeTab === 'reports' ? 'text-blue-700 border-b-4 border-blue-700' : 'text-slate-400'}`}>Archived Reports</button>
            </div>
            {activeTab !== 'reports' && (
              <div className="flex gap-3 pb-4">
                <input className="bg-white border-2 border-slate-200 rounded-xl px-4 py-1.5 text-xs font-black" placeholder="Snapshot Title..." value={reportTitle} onChange={e => setReportTitle(e.target.value)} />
            ...
            <div className="space-y-10">
                ...
                <div className="lg:col-span-4 bg-white border-2 border-slate-100 p-8 rounded-[2.5rem] shadow-sm">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="pt-4 border-t-2 border-slate-50">
                       <p className="text-xs font-black uppercase text-slate-500 mb-1">Country Surplus Gap</p>
                       <p className="text-3xl font-black text-red-700 font-mono tracking-tighter">{Math.round(auditResult.nat.actual - auditResult.nat.tobacco).toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>

              {/* RESTORED AUDIT GUIDES */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-10">
                <div className="bg-white p-10 rounded-[2rem] border-2 border-slate-200">
                  ...
                  <p className="text-sm leading-relaxed text-slate-600 font-bold">Reported tobacco imports represent the maximum legal production capacity. If exports exceed this, finished goods are likely being produced from unrecorded raw leaf.</p>
                </div>
                <div className="bg-white p-10 rounded-[2rem] border-2 border-slate-200">
                  <h3 className="text-slate-900 font-black text-sm mb-4 flex items-center gap-2 uppercase tracking-wide"><Calculator size={22} className="text-blue-700"/> 2. Precursor Rule</h3>
                  <p className="text-sm leading-relaxed text-slate-600 font-bold">The system uses the "Limiting Factor" (minimum of all precursor types). If an entity has 0 Tobacco, their capacity is automatically 0.</p>
                </div>
              </div>
            </div>
          ) : activeTab === 'entities' ? (
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 flex items-center gap-4 bg-white p-4 rounded-2xl border-2 border-slate-200 shadow-sm">
                  ...
                  <input className="w-full outline-none font-bold text-black" placeholder="Filter entities by name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                </div>
                <div className="bg-slate-900 text-white px-8 py-4 rounded-2xl flex items-center gap-10 shadow-lg">
                   <div className="text-center">
                     <p className="text-[10px] font-black uppercase text-slate-400">Filtered TX</p>
                     <p className="font-mono font-black">{filteredSums.tx.toLocaleString()}</p>
                   </div>
                   <div className="text-center">
                     <p className="text-[10px] font-black uppercase text-slate-400">Total Actual</p>
                        ...
                        <td className="p-8">
                          <div className="flex flex-wrap gap-3">
                            {Object.entries(e.materials).map(([m, s]) => (
                              <div key={m} className="bg-white border-2 border-slate-200 rounded-xl px-4 py-2 flex items-center gap-3 text-black">
                                {Icons[m]}
                                <span className="font-mono font-black text-sm">{Math.round(s.rawQty).toLocaleString()} <span className="text-[10px] font-bold">{s.unit}</span></span>
                              </div>
                            ))}
                          </div>
                        </td>
                                  ...
                                  <div className="bg-white border-2 border-red-500 p-6 rounded-2xl shadow-2xl text-left">
                                    ...
                                    <p className="text-xs text-black leading-relaxed font-bold">
                                      {e.violationType === 'ZERO_TOBACCO' 
                                        ? `CRITICAL: Exporting ${Math.round(e.actual).toLocaleString()} sticks with ZERO recorded tobacco.`
    ...
    <div className="flex justify-between items-end">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-lg font-black text-black">{Math.round(kg).toLocaleString()} <span className="text-[10px] font-bold uppercase">{unit}</span></p>
        </div>
      </div>
