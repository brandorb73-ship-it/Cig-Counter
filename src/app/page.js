...
import { ShieldAlert, Activity, Database, Wind, FileText, Pipette, Trash2, Calculator, AlertTriangle, RefreshCcw, Save, History, Search, Info, Sliders, CheckCircle, Target, Gavel, Zap, Download, XCircle, ChevronRight, HelpCircle, Landmark, TrendingUp, Fingerprint, EyeOff } from 'lucide-react';
...
export default function ForensicGradeV10() {
  ...
  const auditResult = useMemo(() => {
    ...
    return { 
        ...
        ].filter(p => p.val > 0).reduce((p, c) => p.val < c.val ? p : c, {name: 'None', val: 0}),
        tobaccoCeiling: nat.tobacco, 
        productionGap, 
  ...
  const downloadCSV = () => {
    const data = filteredEntities.map(e => ({
      Entity: e.name,
      Transactions: e.tx,
      'Reliability Score %': e.reliability.toFixed(1),
    ...
    link.download = `Forensic_Report_V10_${new Date().toISOString().slice(0,10)}.csv`;
    link.click();
  };
    ...
    <div className="min-h-screen bg-slate-50 text-black p-6 lg:p-10 font-sans">
      ...
      <div className="max-w-[1600px] mx-auto mb-8 flex flex-col lg:flex-row items-center gap-6 bg-white border border-slate-300 p-6 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4 mr-auto">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-black uppercase">Forensic Monitor <span className="text-blue-700">10.0</span></h1>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-1"><CheckCircle size={10} className="text-emerald-600"/> Intelligence Suite</p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="bg-slate-900 border-2 border-slate-800 p-6 rounded-3xl shadow-lg flex flex-col justify-center relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex items-center gap-2 mb-1"><EyeOff size={14} className="text-red-400"/><p className="text-[10px] font-black text-white uppercase">Shadow Market</p></div>
                        <p className="text-3xl font-black text-white">{Math.round(auditResult.shadowProb)}%</p>
                        <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase">EST. ILLICIT PROBABILITY</p>
                    </div>
                    <div className="absolute right-[-20px] bottom-[-20px] opacity-20"><ShieldAlert size={100} className="text-red-600" /></div>
                </div>
              </div>
            ...
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        <button onClick={downloadCSV} className="bg-slate-900 text-white p-2.5 rounded-xl hover:bg-blue-700 transition-all flex items-center gap-2 px-4 whitespace-nowrap"><Download size={18}/><span className="text-[10px] font-black uppercase tracking-widest">CSV</span></button>
                    </div>
                </div>
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-slate-900 text-white uppercase font-black tracking-widest text-[10px] sticky top-0 z-20">
                    <tr>
                      ...
                      <th className="p-8">Inventory (Sticks Eq)</th>
                    ...
                    {filteredEntities.map((e, i) => (
                      <tr key={i} className="hover:bg-blue-50/50 group/row relative">
                        ...
                        <td className="p-8 text-center">
                            <div className="inline-block px-3 py-1 rounded bg-slate-50 border border-slate-200 text-[10px] font-black font-mono">{e.reliability.toFixed(1)}%</div>
                        </td>
                        <td className="p-8">
                        ...
                        <td className="p-8 text-center relative overflow-visible">
                           <div className="group/risk relative inline-block">
                              ...
                              {/* RESTORED V9.8 LOGIC WITH V9.7 OVERFLOW FIX */}
                                ...
                                <div className={`bg-white border-2 p-6 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] ${e.risk === 'CRITICAL' ? 'border-red-500' : 'border-emerald-500'}`}>
                                  ...
                                  <p className="text-xs text-black leading-relaxed font-bold">
                                    {e.risk === 'CRITICAL' ? (
                                        ...
                                        ? "CRITICAL: Entity is exporting finished sticks but has zero recorded tobacco leaf records. Suggests undisclosed illicit sourcing."
