...
export default function ObsidianPrimeFinal() {
  ...
  const auditResult = useMemo(() => {
    ...
    return { 
      entities, nat, productionGap, leakageData,
  ...
  const downloadCSV = () => {
    ...
    const csv = Papa.unparse(auditResult.entities.map(e => ({
      ...
      Precursor_Cap: Math.round(e.minPot), Actual_Exports: Math.round(e.actual), Risk: e.risk
    })));
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `Forensic_Audit_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  };
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          ...
          {rawData.length > 0 && (
            <div className="flex gap-2">
                <button onClick={downloadCSV} className="bg-white text-slate-700 p-2.5 rounded-xl border border-slate-200 hover:bg-slate-50"><Download size={20}/></button>
                <button onClick={clearSession} className="bg-red-50 text-red-600 p-2.5 rounded-xl border border-red-200"><Trash2 size={20}/></button>
            </div>
          )}
        </div>
      </div>
          ...
          {activeTab === 'country' ? (
            <div className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="bg-slate-900 border-2 border-slate-800 p-6 rounded-3xl shadow-xl flex flex-col justify-center overflow-hidden group">
                    <div className="relative z-10"><p className="text-[10px] text-white uppercase tracking-widest font-black"><EyeOff size={14} className="inline mr-1"/> Shadow Market</p><p className="text-4xl font-black text-white">{Math.round(auditResult.shadowProb)}%</p></div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white border border-slate-200 p-10 rounded-[2.5rem] shadow-sm">
                  <h2 className="text-sm font-black text-black uppercase tracking-widest mb-10 flex items-center gap-2"><Activity size={20} className="text-blue-700"/> Material vs Output</h2>
                ...
                <div className="bg-white border border-slate-200 p-10 rounded-[2.5rem] shadow-sm">
                  <h2 className="text-sm font-black text-black uppercase tracking-widest mb-10 flex items-center gap-2"><Target size={20} className="text-red-600"/> Leakage Distribution</h2>
                  <div className="h-[350px]">
                    <ResponsiveContainer>
                      <PieChart>
                        <Pie data={auditResult.leakageData} innerRadius={80} outerRadius={120} paddingAngle={5} dataKey="value">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] shadow-xl">
                  ...
                  <div className="space-y-6 text-sm leading-relaxed text-blue-50 font-medium">
                    <p>Total exports stand at <span className="text-emerald-400 font-black">{Math.round(auditResult.nat.actual).toLocaleString()}</span> sticks. Cap is limited by <span className="text-blue-400 font-black underline uppercase">{auditResult.bottleneck.name}</span>.</p>
                    <p className="bg-red-950/30 p-4 border-l-4 border-red-500 rounded-r-xl font-bold">Gap of <span className="text-red-400">{auditResult.productionGap.toLocaleString()}</span> sticks = <span className="text-red-400">${Math.round(auditResult.taxLoss).toLocaleString()}</span> fiscal leakage.</p>
                    <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700">
                      {auditResult.nat.actual > auditResult.nat.tobacco ? (
                        <p className="text-red-400 font-bold italic flex gap-2"><AlertTriangle size={18}/> High Risk: Production exceeds leaf by {Math.round((auditResult.nat.actual/auditResult.nat.tobacco - 1)*100)}%.</p>
                      ) : (
                        <p className="text-emerald-400 font-bold italic flex gap-2"><CheckCircle size={18}/> Reconciled: Volumes within precursor limits.</p>
                      )}
                    </div>
                  </div>
                </div>
                <div className="bg-white border-2 border-slate-200 p-10 rounded-[2.5rem] shadow-sm flex flex-col justify-center">
                   ...
                   <div className="grid grid-cols-2 gap-8">
                    <div className="border-l-2 border-slate-100 pl-6"><p className="text-[10px] font-black text-slate-400 uppercase">Precursor Utilization</p><p className="text-2xl font-black text-black">{auditResult.nat.actual > 0 ? Math.min(100, (auditResult.nat.tobacco / auditResult.nat.actual) * 100).toFixed(1) : 0}%</p></div>
                    <div className="border-l-2 border-slate-100 pl-6"><p className="text-[10px] font-black text-slate-400 uppercase">Audit Integrity</p><p className="text-2xl font-black text-blue-700">{auditResult.entities.filter(e => e.reliability > 80).length} / {auditResult.entities.length}</p></div>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
                <div className="flex bg-white p-4 rounded-3xl border border-slate-200">
                    <div className="relative w-full md:w-96"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-2.5 pl-12 pr-4 text-sm font-bold focus:border-blue-600 outline-none" placeholder="Search Target..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                </div>
              <div className="bg-white border-2 border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-slate-900 text-white uppercase font-black tracking-widest text-[10px]">
                    <tr><th className="p-8">Target Entity</th><th className="p-8 text-center">TX</th><th className="p-8 text-center">Reliability</th><th className="p-8">Materials</th><th className="p-8 text-right">Potential</th><th className="p-8 text-right text-emerald-400">Actual</th><th className="p-8 text-center">Verdict</th></tr>
                  </thead>
                        ...
                        <td className="p-8 text-center">
                            <div className="group/rel relative inline-block cursor-help">
                                ...
                                <div className="invisible group-hover/rel:visible opacity-0 group-hover/rel:opacity-100 absolute top-full left-1/2 -translate-x-1/2 mt-2 z-[100] w-64 bg-slate-900 text-white p-5 rounded-xl text-[10px] font-medium leading-relaxed shadow-2xl transition-all">
                                    ...
                                    Analyzes precursor deviation. Low scores indicate missing leaf or tow records, suggesting shadow sourcing.
                                </div>
                            </div>
                        </td>
                        <td className="p-8">
                            ...
                            {Object.entries(e.materials).map(([m, s]) => (
                                <div key={m} className="group/pop relative bg-white border border-slate-200 rounded-xl px-3 py-1.5 flex items-center gap-2 cursor-help">
                                    ...
                                    <div className="invisible group-hover/pop:visible opacity-0 group-hover/pop:opacity-100 absolute bottom-full left-0 mb-3 z-[60] bg-slate-950 text-white p-4 rounded-xl shadow-2xl min-w-[200px] border border-slate-800 transition-all">
                                        <p className="text-blue-400 font-black text-[10px] uppercase mb-1 flex items-center gap-2"><Calculator size={12}/> Conversion Engine</p>
                                        <div className="flex justify-between font-mono text-[10px] mb-1"><span>Input:</span> <span>{Math.round(s.rawQty).toLocaleString()} {s.unit}</span></div>
                                        <div className="flex justify-between font-mono text-[10px]"><span>Potential:</span> <span>{Math.round(s.sticks).toLocaleString()} Sticks</span></div>
                                    </div>
                                </div>
                            ))}
                          </div>
                        ...
                        <td className="p-8 text-center">
                            <div className="group/risk relative inline-block">
                                ...
                                <div className="invisible group-hover/risk:visible opacity-0 group-hover/risk:opacity-100 absolute top-full right-0 mt-2 z-[100] w-80 bg-white border-2 border-slate-900 p-6 rounded-2xl shadow-2xl text-left transition-all">
                                    <p className="font-black text-xs mb-3 uppercase border-b pb-2"><Info size={16} className="inline mr-2"/> Audit Evidence</p>
                                    <p className="text-xs text-black leading-relaxed font-bold">
                                        {e.risk === 'CRITICAL' ? (e.violationType === 'ZERO_TOBACCO' ? "ZERO TOBACCO: Verified exports found with NO matching leaf records. Direct evidence of illicit sourcing." : `OVER CAP: Exports exceed precursor limit by ${Math.round((e.actual/e.minPot - 1)*100)}%.`) : "RECONCILED: Production is within precursor limits."}
                                    </p>
                                </div>
                            </div>
                        </td>
                  ...
                  <tfoot className="bg-slate-50 border-t-4 border-slate-900 sticky bottom-0 z-20">
                    <tr className="font-black text-black">
                      ...
                      <td className="p-8 text-center text-[10px] text-slate-400 uppercase">Aggr Metrics</td>
                      <td className="p-8"></td>
                      ...
                      <td className="p-8 text-center"><div className="text-[10px] bg-slate-900 text-white py-2 px-4 rounded-lg inline-block uppercase tracking-widest font-black">Summary</div></td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {reports.map((r) => (
                <div key={r.id} className="bg-white border-2 border-slate-200 p-8 rounded-[2rem] shadow-sm hover:border-blue-600 transition-all">
                   ...
                   <div className="space-y-4 mb-8 border-y py-6 border-slate-50"><div className="flex justify-between text-xs font-bold"><span>Gap</span> <span className="text-red-600 font-mono">{Math.round(r.gap).toLocaleString()}</span></div><div className="flex justify-between text-xs font-bold"><span>Shadow</span> <span className="text-blue-700 font-mono">{Math.round(r.prob)}%</span></div></div>
